...
export function LightWavesBackground({
  className,
  children,
  colors = ["#59F3E9", "#35D0D9", "#2F6F8F", "#5A6D86", "#101A2C"],
  speed = 0.5,
  intensity = 0.72,
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      bgGradient.addColorStop(0.45, "#07111f")
      ...
      topGlow.addColorStop(
        0,
        `rgba(${hexToRgb(colors[0]).r}, ${hexToRgb(colors[0]).g}, ${hexToRgb(colors[0]).b}, ${0.045 * intensity})`,
      )
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at center, transparent 0%, transparent 52%, rgba(2,8,20,0.82) 100%)",
        }}
      />

      {/* Content layer */}
      {children && <div className="pointer-events-auto relative z-10 h-full w-full">{children}</div>}
    </div>
  )
}
